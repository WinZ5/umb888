...
const ConfigUmbrellaForm = () => {
  ...
  useEffect(() => {
    ...
    const fetchUmbrellaData = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error('Failed to fetch umbrella data');
        }
        const data: UmbrellaFormData = await response.json();
        setFormData(data);
      } catch (err) {
        setError(err instanceof Error ? err.message : 'An error occurred while fetching umbrella data');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to update umbrella');
      }

      const result = await response.json();
      console.log('Umbrella updated:', result);
      handleBack();
    } catch (error) {
...
export default ConfigUmbrellaForm;
