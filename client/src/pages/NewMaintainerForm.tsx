...
const NewMaintainerForm = () => {
  const navigate = useNavigate();
  const [formData, setFormData] = useState<MaintainerFormData>({
    FirstName: '',
    LastName: '',
    Phone: '',
    Email: '',
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to add maintainer');
      }

      const result = await response.json();
      console.log('Maintainer added:', result);
      handleBack();
    } catch (error) {
...
export default NewMaintainerForm;